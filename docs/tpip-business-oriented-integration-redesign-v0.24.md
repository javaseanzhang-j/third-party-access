...
接入服务详情页增加产品化的自动检查，不再要求配置人员逐张技术资产表确认完整性。打开服务详情时，页面调用：

```text
GET /control/v1/product-model/services/{serviceId}/readiness
```

检查结果分为“可以验证”“可以验证，有提示”“暂不能验证”三种。服务级检查包括服务状态、标准请求、标准返回、可用第三方实现和多实现调用选择规则；每个第三方实现继续检查可执行配置、第三方报文结构、调用地址与请求方式、接入通道、通道认证、请求字段转换和返回字段转换。

判定规则如下：

- 一个服务只有一个启用实现时固定使用该实现，不强制建立路由；有多个启用实现时，必须存在覆盖所有启用实现的已发布路由；
- 缺少可执行版本、报文结构、地址、通道或双向映射属于阻断项；
- 通道认证可以为空，以兼容公开匿名接口，但页面会明确提示用户确认；
- 所有阻断项消除后，详情页才显示“进入验证与发布”入口；
- 每个未通过项目都携带业务页面路径，用户可以直接返回“接入服务”或“第三方接入”补齐。

该检查只判断设计态配置是否具备验证条件，不替代 FixtureSuite、Workspace Verification、Bundle 编译和 Deployment 激活。它是业务配置与原发布治理底座之间的前置门禁。
