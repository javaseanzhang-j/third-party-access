...
> MCP Tool已进入Control Plane资产管理，后端配置与发布步骤见 [MCP Tool资产配置与发布指南 v0.2](./tpip-mcp-tool-asset-configuration-v0.2.md)。
...
工具详情可对每个不可变版本执行“检查契约变化”。删除字段、字段类型改变、新增请求必填字段会被标记为不兼容，必须创建新版本；新增选填字段和返回字段会作为可兼容变化提示。右上角“客户端接入”提供本机 MCP 地址和通用 Streamable HTTP 配置参考，不会把调用方 Secret 暴露给客户端。

“本地调用测试”展示当前固定应用身份和已经发布且获得服务授权的工具。新发布、撤销或授权变化默认30秒内自动进入运行目录，也可以手动点击“立即刷新目录”，不需要重启MCP Server。测试台根据工具输入Schema生成JSON示例，调用前对有副作用或需要确认的工具再次提示；执行属于真实调用，会继续经过Runtime、Bundle、路由和第三方接口，并返回可追踪的requestId。该入口只接受本机回环请求。
