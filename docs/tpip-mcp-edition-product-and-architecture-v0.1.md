...
第四阶段契约影响与本地联调辅助已完成：Control Plane可以按不可变Tool版本对比业务服务最新已发布请求/返回契约，区分一致、可兼容新增、不兼容变化和无法分析，并输出字段路径与处理建议；AI工具工作台展示分析结果，并提供本机MCP地址、接入前置步骤和可复制的通用Streamable HTTP客户端配置参考。

第五阶段运行目录与本地调用闭环已完成：MCP Server使用可原子替换的Tool目录，默认每30秒从Control Plane重新加载已发布Tool并重新计算应用授权交集；也支持工作台手动刷新。刷新成功后同步更新MCP SDK工具注册并发送 `tools/list_changed` 通知，刷新失败则保留上一份可用快照和安全状态信息。AI工具工作台新增“本地调用测试”，仅显示当前应用有权使用的工具，按输入Schema生成示例参数，并通过同一 `McpToolGatewayService` 和Runtime执行真实调用。测试HTTP入口强制只允许回环地址访问，仍不暴露Secret原文。

当前明确限制只剩本机固定身份，不支持远程OAuth/OIDC客户端；这是后续多人共享和远程部署阶段的前置能力，不影响当前单人本地使用。
