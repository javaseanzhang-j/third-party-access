...
const versionForm = reactive({ semanticVersion: '1.0.0', compatibilityMode: 'BACKWARD' as CompatibilityMode,
...
const createVersion = useMutation({ mutationFn: (input: { contractId: number; schema: Record<string, unknown>; example: unknown | null }) => canonicalAssetApi.createVersion(input.contractId, { semanticVersion: versionForm.semanticVersion, schemaStandard: 'JSON_SCHEMA_2020_12', schemaDocument: input.schema, exampleDocument: input.example, compatibilityMode: versionForm.compatibilityMode }), onSuccess: () => { versionDialog.value = false; void versions.refetch(); ElMessage.success('Contract Version 已创建') }, onError: (e: Error) => { errorMessage.value = message(e) } })
...
function submitVersion(): void {
  errorMessage.value = ''
  if (!selectedContractId.value || !/^(0|[1-9]\d*)\.(0|[1-9]\d*)\.(0|[1-9]\d*)(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/.test(versionForm.semanticVersion)) { errorMessage.value = '请选择 Contract，并填写合法语义版本。'; return }
  try {
...
<template>
  <section>
    <div class="page-heading"><div><h2>业务标准接口</h2><p>定义业务稳定语义和 Canonical JSON Contract，隔离第三方字段与协议变化。</p></div><el-tag effect="plain">Canonical Model</el-tag></div>
    <el-alert v-if="errorMessage" class="command-notice" type="error" :title="errorMessage" closable @close="errorMessage = ''" show-icon />
    <el-alert class="command-notice" type="info" :closable="false" title="先定义业务语义，再配置第三方适配。Contract Version 发布后不可修改，变化必须创建新语义版本。" show-icon />
    <div class="surface integration-asset-tabs canonical-tabs">
      <el-tabs v-model="activeTab">
        <el-tab-pane label="Domain" name="domains">
          <div class="asset-toolbar"><div><strong>业务域</strong><span>按业务责任边界组织能力，不按第三方厂商划分。</span></div><el-button type="primary" @click="domainDialog = true">新增 Domain</el-button></div>
        ...
        <el-tab-pane label="Capability" name="capabilities">
        ...
        <el-tab-pane label="Operation" name="operations">
        ...
        <el-tab-pane label="CanonicalContract" name="contracts">
        ...
        <el-tab-pane label="ContractVersion" name="versions">
    ...
    <el-dialog v-model="versionDialog" title="新增 Canonical Contract Version" width="820px"><el-alert type="warning" :closable="false" title="发布后不可修改。请先通过样例确认 Schema，再执行发布。" show-icon /><el-form class="dialog-form" label-position="top"><div class="form-two-columns"><el-form-item label="语义版本" required><el-input v-model="versionForm.semanticVersion" placeholder="1.0.0" /></el-form-item><el-form-item label="兼容策略" required><el-select v-model="versionForm.compatibilityMode"><el-option v-for="item in (['NONE','BACKWARD','FORWARD','FULL'] as CompatibilityMode[])" :key="item" :value="item" /></el-select></el-form-item></div><el-form-item label="JSON Schema 2020-12" required><el-input v-model="versionForm.schemaText" type="textarea" :rows="12" class="schema-editor" /></el-form-item><el-form-item label="Example JSON（可选）"><el-input v-model="versionForm.exampleText" type="textarea" :rows="6" class="schema-editor" /></el-form-item></el-form><template #footer><el-button @click="versionDialog = false">取消</el-button><el-button type="primary" :loading="createVersion.isPending.value" @click="submitVersion">创建 DRAFT</el-button></template></el-dialog>
  </section>
</template>
