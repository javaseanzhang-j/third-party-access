...
const createContract = useMutation({ mutationFn: (input: CreateProviderContractInput) => integrationAssetApi.createContract(input), onSuccess: () => { contractDialog.value = false; invalidate(); ElMessage.success('ProviderContract 已创建') }, onError: (e: Error) => { errorMessage.value = message(e) } })
...
function submitEndpoint(): void {
  errorMessage.value = ''
  if (!endpointForm.providerContractId || !requireCode(endpointForm.endpointCode) || !endpointForm.baseUrl.trim() || !endpointForm.resourcePath.startsWith('/')) { errorMessage.value = '请选择 ProviderContract，并填写合法 Endpoint 编码、Base URL 和以 / 开头的资源路径。'; return }
...
<template>
  <section>
    <div class="page-heading"><div><h2>第三方系统配置</h2><p>管理 Provider、Secret 引用、第三方协议和环境化 Endpoint Revision。</p></div><el-tag effect="plain">本地配置模式</el-tag></div>
    ...
    <div class="surface integration-asset-tabs">
      <el-tabs v-model="activeTab">
        ...
        <el-tab-pane label="ProviderContract" name="contracts">
          <div class="asset-toolbar"><div><strong>第三方协议身份</strong><span>先创建稳定 Contract；请求、响应和错误 Schema 将在下一阶段通过不可变版本配置。</span></div><el-button type="primary" :disabled="!providerItems.length" @click="contractDialog = true">新增 ProviderContract</el-button></div>
          <AsyncStatePanel :loading="contracts.isPending.value" :error="contracts.isError.value" error-title="ProviderContract 读取失败" @retry="contracts.refetch()">
            <el-empty v-if="!contractItems.length" description="尚未创建 ProviderContract" />
            <el-table v-else :data="contractItems" row-key="id"><el-table-column label="Contract" min-width="260"><template #default="{ row }"><strong>{{ row.contractName }}</strong><div class="subtle mono">{{ row.contractCode }}</div></template></el-table-column><el-table-column label="Provider" min-width="220"><template #default="{ row }">{{ providerName(row.providerId) }}</template></el-table-column><el-table-column prop="protocolType" label="协议" width="110" /><el-table-column prop="status" label="状态" width="100" /></el-table>
          </AsyncStatePanel>
        </el-tab-pane>
    ...
    <el-dialog v-model="contractDialog" title="新增 ProviderContract" width="580px"><el-form label-position="top" @submit.prevent="submitContract"><el-form-item label="Provider" required><el-select v-model="contractForm.providerId" filterable style="width:100%"><el-option v-for="item in providerItems" :key="item.id" :label="providerName(item.id)" :value="item.id" /></el-select></el-form-item><el-form-item label="Contract 编码" required><el-input v-model="contractForm.contractCode" placeholder="vendor.customer.lookup" /></el-form-item><el-form-item label="名称" required><el-input v-model="contractForm.contractName" /></el-form-item><el-form-item label="协议" required><el-select v-model="contractForm.protocolType" style="width:100%"><el-option v-for="item in (['HTTP','SOAP','GRAPHQL','GRPC'] as ProtocolType[])" :key="item" :value="item" /></el-select></el-form-item><el-form-item label="说明"><el-input v-model="contractForm.description" type="textarea" /></el-form-item><p v-if="errorMessage" class="command-validation">{{ errorMessage }}</p></el-form><template #footer><el-button @click="contractDialog = false">取消</el-button><el-button type="primary" :loading="createContract.isPending.value" @click="submitContract">创建</el-button></template></el-dialog>
    <el-dialog v-model="endpointDialog" title="新增 Endpoint Revision" width="720px"><el-form label-position="top" @submit.prevent="submitEndpoint"><el-form-item label="ProviderContract" required><el-select v-model="endpointForm.providerContractId" filterable style="width:100%"><el-option v-for="item in contractItems" :key="item.id" :label="contractName(item.id)" :value="item.id" /></el-select></el-form-item><div class="form-two-columns"><el-form-item label="Endpoint 编码" required><el-input v-model="endpointForm.endpointCode" placeholder="vendor.customer.lookup" /></el-form-item><el-form-item label="环境" required><el-input v-model="endpointForm.environmentCode" /></el-form-item></div><div class="endpoint-address-row"><el-form-item label="协议" required><el-select v-model="endpointForm.protocolScheme"><el-option v-for="item in (['HTTP','HTTPS'] as EndpointScheme[])" :key="item" :value="item" /></el-select></el-form-item><el-form-item label="Method" required><el-select v-model="endpointForm.httpMethod"><el-option v-for="item in (['GET','POST','PUT','PATCH','DELETE'] as EndpointMethod[])" :key="item" :value="item" /></el-select></el-form-item><el-form-item label="Base URL" required><el-input v-model="endpointForm.baseUrl" placeholder="https://api.vendor.example" /></el-form-item></div><el-form-item label="资源路径" required><el-input v-model="endpointForm.resourcePath" placeholder="/vendor/v1/query" /></el-form-item><el-form-item label="CredentialRef"><el-select v-model="endpointForm.credentialRefId" clearable filterable style="width:100%"><el-option v-for="item in credentialItems" :key="item.id" :label="`${item.credentialCode} · ${item.environmentCode}`" :value="item.id" /></el-select></el-form-item><div class="form-three-columns"><el-form-item label="连接超时 ms"><el-input-number v-model="endpointForm.connectTimeoutMs" :min="1" /></el-form-item><el-form-item label="读取超时 ms"><el-input-number v-model="endpointForm.readTimeoutMs" :min="1" /></el-form-item><el-form-item label="总超时 ms"><el-input-number v-model="endpointForm.totalTimeoutMs" :min="1" /></el-form-item></div><p v-if="errorMessage" class="command-validation">{{ errorMessage }}</p></el-form><template #footer><el-button @click="endpointDialog = false">取消</el-button><el-button type="primary" :loading="createEndpoint.isPending.value" @click="submitEndpoint">创建 Revision</el-button></template></el-dialog>
  </section>
</template>
