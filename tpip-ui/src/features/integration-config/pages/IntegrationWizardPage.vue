  ...
  <section>
    ...
    <div class="surface wizard-context-panel">
      ...
      <div class="wizard-context-grid">
        ...
        <label><span>Provider Contract</span><el-select v-model="draft.providerContractId" filterable placeholder="先创建第三方协议" @change="contextChanged"><el-option v-for="item in providerContractItems" :key="item.id" :label="`${item.contractName} · ${item.contractCode}`" :value="item.id" /></el-select></label>
