...
const appKey=ref('');const appSecret=ref('');const scenario=ref('')
...
const invoke = useMutation({ mutationFn: () => {
  ...
  return runtimeInvokeApi.invoke(currentOperation.value!.operationCode, { meta: { requestId: requestId.value.trim(), caller: caller.value.trim(), tenantId: tenantId.value.trim() || null, idempotencyKey: idempotencyKey.value.trim() || null, deadline: deadline.value.trim() || null, attributes }, payload: parseObject(payloadText.value, 'Payload') },{appKey:appKey.value.trim(),appSecret:appSecret.value,scenario:scenario.value.trim()||null})
...
function submit(): void {
  ...
  if (!currentOperation.value || !requestId.value.trim() || !caller.value.trim()||!appKey.value.trim()||!appSecret.value) { errorMessage.value = 'Operation、Request ID、Caller、App Key 和本地签名密钥均为必填项。'; return }
...
<template>
  <section>
    ...
    <div class="runtime-invoke-grid">
      <div class="surface runtime-request-panel"><div class="section-title"><h3>业务标准调用请求</h3><el-button @click="resetRequestId">生成 Request ID</el-button></div><el-form class="runtime-request-form" label-position="top"><el-alert type="warning" :closable="false" title="本地签名密钥只保存在当前页面内存中，用于生成HMAC，不会提交给Control Plane。"/><div class="form-two-columns"><el-form-item label="调用方 App Key" required><el-input v-model="appKey" placeholder="从调用方管理复制已发布App Key"/></el-form-item><el-form-item label="本地签名密钥" required><el-input v-model="appSecret" type="password" show-password autocomplete="off"/></el-form-item><el-form-item label="业务场景（按授权填写）"><el-input v-model="scenario" placeholder="例如 login-verification"/></el-form-item><el-form-item label="接入服务" required><el-select v-model="selectedOperationId" filterable><el-option v-for="item in operationItems" :key="item.id" :label="`${item.operationName} · ${item.operationCode}`" :value="item.id" /></el-select></el-form-item><el-form-item label="Runtime环境"><el-input v-model="environmentCode" /></el-form-item><el-form-item label="Request ID" required><el-input v-model="requestId" /></el-form-item><el-form-item label="调用方说明" required><el-input v-model="caller" /></el-form-item><el-form-item label="租户ID（可选）"><el-input v-model="tenantId" clearable /></el-form-item><el-form-item label="幂等键（可选）"><el-input v-model="idempotencyKey" clearable /></el-form-item><el-form-item label="截止时间 ISO-8601（可选）"><el-input v-model="deadline" placeholder="2026-08-09T12:00:00Z" clearable /></el-form-item></div><div class="runtime-json-editors"><el-form-item label="调用属性（仅字符串值，不得包含Secret）"><el-input v-model="attributesText" type="textarea" :rows="8" class="schema-editor" /></el-form-item><el-form-item label="业务标准报文"><el-input v-model="payloadText" type="textarea" :rows="12" class="schema-editor" /></el-form-item></div><el-button type="primary" :disabled="!activeRoute.data.value" :loading="invoke.isPending.value" @click="submit">签名并调用 Runtime</el-button></el-form></div>
