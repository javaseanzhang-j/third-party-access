...
class BusinessRequestPreviewAssemblerTest {
    @Test void createsHumanReadablePreviewAndNeverExposesSecretMaterial() {
        ...
        var auth = new ChannelAuthenticationVersion(4L, 1, 1, 5, 6, "{\"headerName\":\"Authorization\"}",
        ...
        var item = CredentialProfileItem.secretRef(6, AssetCode.of("access-key-secret"), "AccessKey Secret", 8, null);
        ...
        var preview = new BusinessRequestPreviewAssembler(json).assemble(channel, contract, transport,
                auth, template, templateVersion, profile, List.of(item), Map.of(8L, ref));

        assertEquals("https://dysmsapi.aliyuncs.com/", preview.target().finalUrl());
        assertEquals("Authorization", preview.authentication().effects().getFirst().name());
        assertTrue(preview.authentication().credentialFields().getFirst().displayValue().contains("未读取明文"));
